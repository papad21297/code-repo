...
int		**permutation_iteration(
			int n, int formation_number, int cond_arr[], int *ptr_amount);
	...
	int	*ptr_amount)
{
	...
	while (i < formation_number)
	{
		...
		matrix[i] = val_arr;
		if ((cond_arr[0] == prmttion_mmber_itration(val_arr, n)) && (cond_arr[1]
